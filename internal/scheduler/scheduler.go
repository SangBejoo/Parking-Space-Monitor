	...
	for _, taxi := range taxis {
		...
		for _, place := range places {
			...
			if isPointInPolygon(taxi.Longitude, taxi.Latitude, polygon) {
				...
				err := s.Repo.MappingRepo.UpsertMapping(taxi.TaxiID, place.PlaceID)
				if err != nil {
					log.Printf("Error upserting mapping for taxi %s: %v", taxi.TaxiID, err)
				}

				// Update counter
				err = s.Repo.MappingRepo.UpdateCounter(taxi.TaxiID, place.PlaceID)
				if err != nil {
					log.Printf("Error updating counter for taxi %s: %v", taxi.TaxiID, err)
				}
